...
2) При загрузке системы зажимаем ```Shift```, чтобы попасть к меню загрузки ```Grub```
     ...
     Если очень коротко, то Это функция, которая используется для обслуживания системы: она позволяет сисадмину восстанавливать систему из испорченных файлов 	      инициализации или менять забытый пароль.
