...
1) Запустив дистрибутив мы можем обнаружить что на этот раз ip-адрес не высвечивается как это было в snow-crash, поэтому нужно будет его найти.
   ...
   - Вспомним что такое ```curl```?
     ...
     У нас сотался только SSH протокол, давайте попробуем зайти под логином ```laurie``` и с паролем ```Iheartpwnage```, вот тут забегая вперед скажу что этого        сделать не получится. Как оказалось пароль который мы получили нужно захешировать ```echo -n "Iheartpwnage" | shasum -a 256```, это мы поняли только спустя 	время и подсказки, теперь мы можем залогиниться под логином ```laurie``` с паролем ```330b845f32185747e4f8ca15d40ca59796035c89ea809fb5d30f4da83ecf45a4```
