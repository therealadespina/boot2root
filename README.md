...
1) Запустив дистрибутив мы можем обнаружить что на этот раз ip-адрес не высвечивается как это было в snow-crash, поэтому нужно будет его найти.
   ...
   - Теперь нам нужно убедиться что мы действительно находимся в одной подсети, запускаем наши машины и все операции на этот раз мы будем производить с Kali,          аккуратный способ сделать это - пропинговать широковещательный адрес, который заполнит наш локальный кэш arp таблицы.
     ...
     arp -a
     ```
     С нашей Kali можно сразу воспользоваться утилитой nmap для поиска уязвимостей и вбить следующее: ```nmap 192.168.56.1-255```
     тем самым проверить на уязвимость все устройства которые находятся с нами в одной подсети и перезаписать ```arp``` таблицу, если что, то BornToSecHackMe-      v1.1 там засветится.
   ...
   - Вспомним что такое ```curl```?
     ...
     ```
     Это не весь код, смотри ```dirty.c```, дальше ты знаешь что делать. Enjoy!
     
2) При загрузке системы зажимаем ```Shift```, чтобы попасть к меню загрузки ```Grub```

     Перед тем как что-то вводить нажми на ```tab``` чтобы посмотреть какое ядро доступно
     
     Набираем: ```live init=/bin/bash```
     
     Если очень коротко, то эта функция, используется для обслуживания системы она позволяет восстанавливать систему из испорченных файлов 	      	      	      инициализации или менять забытый пароль.
...
3) Уязвимость Apache suEXEC
   ...
   - Имея возможность запуска suexec с передачей параметров командной строки (обычно, запуск разрешен только с uid httpd процесса), злоумышленник может создать      файлы c ```UID``` и ```GID``` других пользователей.
     ...
     После исследования мы обнаружили, что эта версия уязвима. Одна из уязвимостей которую мы будем использовать - это обход пути. Мы можем создать                  символическую ссылку с ```/``` и страницу ```php``` для доступа к файлам, которые пользователь ```www-data``` может читать.
     
     Проверить версию: ```nmap -sV 192.168.56.XX```
     
     Снова сделаем SQL инъекцию: ```SELECT 1, '<?php symlink(\"/\", \"paths.php\");?>' INTO OUTFILE '/var/www/forum/templates_c/run.php'```, после этого можно      сразу идти по адресу ```https://192.168.56.XX/forum/templates_c/run.php```, чтобы запустить наш маленький скрипт, как только скрипт отработает мы увидим в      окне браузера ```1```, значит можно идти по следующему адресу ```https://192.168.56.XX/forum/templates_c/paths.php```
     
     Поздравляю, вы получили доступ ко всем каталогам, удачи!
     
4) Остальные реализации получения прав суперпользователя https://dirtycow.ninja/
